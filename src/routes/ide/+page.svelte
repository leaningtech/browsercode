<script lang="ts">
	...
	function applyPortalUpdate(update: PortalUpdate) {
		...
		if (update.active && update.url) {
			...
			const preferred = session.config.appPort;
	...
	onMount(async () => {
		...
		try {
			await session.boot(getFrameworkFromUrl(), { output: outputEl, bash: bashEl }, (update) =>
...
<div class="flex h-full min-h-0 w-full min-w-0 flex-col overflow-hidden text-zinc-300">
	...
	>
		<div class="flex items-center gap-2 text-[11px] text-white/40">
			<span class="text-white/60">{session.config.label}</span>
		...
		{#if activePanel}
			...
			>
				...
				<div class="flex-1 overflow-y-auto p-1.5">
					...
					{:else}
						<FrameworkPanel selected={session.framework} onSelect={selectFramework} />
					{/if}
				</div>
			</div>
